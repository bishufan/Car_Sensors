...
namespace lslidar_driver {
    ...
    lslidarDriver::lslidarDriver(ros::NodeHandle &node, ros::NodeHandle &private_nh) : nh(node),
        ...
        // 初始化Cartographer相关参数
        pointcloud_format = "default";  // 默认使用原始格式
        cartographer_first_frame = true;
        return;
    }
    ...
    bool lslidarDriver::loadParameters() {
        ...
        // 新增：点云格式参数
        pnh.param<std::string>("pointcloud_format", pointcloud_format, "default");
        // "default": 原始格式
        // "cartographer": Cartographer格式
        ...
        ROS_INFO("PointCloud format: %s", pointcloud_format.c_str());
        return true;
    ...
    void lslidarDriver::publishPointcloud() {
        // 根据配置选择点云格式
        if (pointcloud_format == "cartographer") {
            publishPointcloudForCartographer();
            return;
        }
    ...
    // 新增函数：为Cartographer发布的点云格式
void lslidarDriver::publishPointcloudForCartographer() {
    VPointcloud::Ptr point_cloud(new VPointcloud());
    point_cloud->header.frame_id = frame_id;
    point_cloud->height = 1;
    point_cloud->header.stamp = static_cast<uint64_t>(sweep_end_time * 1e6);
    ...
    // 所有点使用固定的负时间戳
    const double fixed_negative_time = -0.1;  // 所有点都是-0.1秒
    
    // 如果是第一帧，设置基准时间
    if (cartographer_first_frame && !sweep_data->points.empty()) {
        cartographer_base_time = ros::Time(sweep_data->points[0].time);
        cartographer_first_frame = false;
        ROS_INFO("Cartographer format: base time set to %.6f", cartographer_base_time.toSec());
    }
    ...
    if (sweep_data->points.size() > 0) {
        for (j = 0; j < sweep_data->points.size(); ++j) {
            // 角度过滤检查
            if(scan_start_angle > scan_end_angle) {
                if ((sweep_data->points[j].azimuth > scan_end_angle) &&
                    (sweep_data->points[j].azimuth < scan_start_angle)) {
                    continue;
                }
            } else {
                if ((sweep_data->points[j].azimuth < scan_start_angle) ||
                    (sweep_data->points[j].azimuth > scan_end_angle)) {
                    continue;
                }
            }
    ...
    if (sweep_data->points.size() > 0) {
        ...
        for (j = 0; j < sweep_data->points.size(); ++j) {
            ...
            // 填充点数据
            point.x = static_cast<float>(sweep_data->points[j].x);
            point.y = static_cast<float>(sweep_data->points[j].y);
            point.z = static_cast<float>(sweep_data->points[j].z);
            point.intensity = static_cast<float>(sweep_data->points[j].intensity);
            
            // =========== 绝对正确的时间戳计算 ===========
            // 方法1：所有点使用固定的负时间戳（最简单可靠）
            point.time = fixed_negative_time;  // 所有点都是-0.1秒
            
            // 方法2：或者使用微小递减的负时间戳（保证严格递减）
            // double base_time = -0.1;
            // double time_increment = -0.000001; // 每个点递减1微秒
            // point.time = base_time + (point_index * time_increment);
            
            // 确保时间戳是负数
            if (point.time > 0) {
                ROS_WARN("时间戳为正数(%.6f)，强制设置为-0.1", point.time);
                point.time = -0.1;
            }
            
            point.ring = static_cast<uint16_t>(sweep_data->points[j].ring);
            
            point_cloud->points.push_back(point);
            point_index++;
            
            // 调试输出前几个点（仅第一次）
            if (scan_count == 1 && point_index <= 5) {
                ROS_INFO("Cartographer点 %zu: x=%.3f, y=%.3f, z=%.3f, time=%.6f, ring=%d",
                         point_index, point.x, point.y, point.z, point.time, point.ring);
            }
        }
    }
    ...
    if (point_cloud->width == 0) {
        ROS_WARN("没有有效的点发布到Cartographer");
        return;
    }
    
    // 检查时间戳
    if (point_cloud->points.size() > 0) {
        double first_time = point_cloud->points[0].time;
        ROS_INFO_ONCE("Cartographer时间戳: 所有点都设置为 %.6f", first_time);
        
        // 验证所有点的时间戳
        for (size_t i = 0; i < point_cloud->points.size(); ++i) {
            if (point_cloud->points[i].time > 0) {
                ROS_ERROR("点 %zu 的时间戳为正数: %.6f，强制修正为-0.1", i, point_cloud->points[i].time);
                point_cloud->points[i].time = -0.1;
            }
        }
    }
    ...
    // =========== 强制修正为Cartographer格式 ===========
    // 总是进行修正，确保格式100%正确
    ROS_DEBUG_ONCE("强制修正为Cartographer格式...");
    
    // 保存原始字段信息用于调试
    std::vector<sensor_msgs::PointField> original_fields = pc_msg.fields;
    
    // 手动设置Cartographer格式字段
    pc_msg.fields.clear();
    ...
    // 重新打包数据，确保格式正确
    std::vector<uint8_t> new_data(pc_msg.row_step * pc_msg.height);
    uint8_t* dst = &new_data[0];
    
    for (size_t i = 0; i < point_cloud->points.size(); ++i) {
        const VPoint& pt = point_cloud->points[i];
        
        float* x_ptr = reinterpret_cast<float*>(dst);
        float* y_ptr = reinterpret_cast<float*>(dst + 4);
        float* z_ptr = reinterpret_cast<float*>(dst + 8);
        float* intensity_ptr = reinterpret_cast<float*>(dst + 12);
        float* time_ptr = reinterpret_cast<float*>(dst + 16);
        uint16_t* ring_ptr = reinterpret_cast<uint16_t*>(dst + 20);
        
        *x_ptr = pt.x;
        *y_ptr = pt.y;
        *z_ptr = pt.z;
        *intensity_ptr = pt.intensity;
        
        // 关键：确保time是float32类型的负数
        float carto_time = static_cast<float>(pt.time);
        if (carto_time > 0) {
            ROS_ERROR("转换后的时间戳仍为正数: %.6f，强制设置为-0.1", carto_time);
            carto_time = -0.1f;
        }
        *time_ptr = carto_time;
        ...
        *ring_ptr = pt.ring;
        ...
        // 调试：检查前几个点的数据
        if (i < 3 && scan_count == 1) {
            ROS_INFO("打包点 %zu: time字段地址=%p, 值=%.6f", 
                     i, time_ptr, *time_ptr);
            
            // 检查内存中的实际值
            uint8_t* time_bytes = dst + 16;
            float time_val;
            memcpy(&time_val, time_bytes, 4);
            ROS_INFO("  内存验证: time=%.6f (应该是负数)", time_val);
        }
        
        dst += 24;
    }
    
    pc_msg.data = new_data;
    
    // 最终验证
    if (scan_count == 1) {
        ROS_INFO("=== 最终格式验证 ===");
        ROS_INFO("字段数量: %zu", pc_msg.fields.size());
        for (size_t i = 0; i < pc_msg.fields.size(); ++i) {
            ROS_INFO("  字段[%zu]: name=%s, offset=%d, datatype=%d", 
                     i, pc_msg.fields[i].name.c_str(),
                     pc_msg.fields[i].offset,
                     pc_msg.fields[i].datatype);
        }
        ROS_INFO("点步长: %d", pc_msg.point_step);
        ROS_INFO("点数: %d", pc_msg.width);
        
        // 检查第一个点的时间戳
        if (pc_msg.width > 0) {
            float first_time;
            memcpy(&first_time, &pc_msg.data[16], 4); // time字段在偏移16
            ROS_INFO("第一个点时间戳: %.6f (应该是负数)", first_time);
            
            if (first_time > 0) {
                ROS_ERROR("错误！第一个点时间戳为正数: %.6f", first_time);
                // 强制修正
                first_time = -0.1f;
                memcpy(&pc_msg.data[16], &first_time, 4);
            }
        }
        ROS_INFO("===================");
    }
    ...
    // 打印一次成功信息
    static bool success_printed = false;
    if (!success_printed) {
        success_printed = true;
        ROS_INFO("✅ Cartographer格式点云发布成功！");
        ROS_INFO("   格式: x,y,z,intensity,time(float32),ring(uint16)");
        ROS_INFO("   偏移: 0,4,8,12,16,20");
        ROS_INFO("   时间戳: 所有点固定为-0.1秒");
    }
}
