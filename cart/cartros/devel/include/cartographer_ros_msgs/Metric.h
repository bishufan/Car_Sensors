  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::cartographer_ros_msgs::Metric_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.labels.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::cartographer_ros_msgs::MetricLabel_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.labels[i]);
    }
    if (v.labels.empty() || false)
      s << "]";
    if (true || !indent.empty())
    ...
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "counts_by_bucket: ";
    if (v.counts_by_bucket.empty() || false)
      s << "[";
    for (size_t i = 0; i < v.counts_by_bucket.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::cartographer_ros_msgs::HistogramBucket_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.counts_by_bucket[i]);
    }
    if (v.counts_by_bucket.empty() || false)
      s << "]";
  }
