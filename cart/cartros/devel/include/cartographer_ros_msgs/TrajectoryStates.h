  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::cartographer_ros_msgs::TrajectoryStates_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.trajectory_id.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<int32_t>::stream(s, true ? std::string() : indent + "    ", v.trajectory_id[i]);
    }
    if (v.trajectory_id.empty() || true)
      s << "]";
    if (true || !indent.empty())
    ...
    if (v.trajectory_state.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.trajectory_state.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<uint8_t>::stream(s, true ? std::string() : indent + "    ", v.trajectory_state[i]);
    }
    if (v.trajectory_state.empty() || true)
      s << "]";
  }
